...
const TestComponent = () => {
  ...
  return (
    <div>
      ...
      <button onClick={() => importSettings('{"themeMode":"dark","language":"fr"}')}>Import Settings</button>
...
describe('SettingsContext', () => {
  ...
  it('updates settings and saves to localStorage', async () => {
    ...
    await waitFor(() => {
      expect(mockStorage.setItem).toHaveBeenCalledWith('settings', expect.stringContaining('"themeMode":"dark"'));
    });
  });
  ...
  it('imports settings from JSON', async () => {
    ...
    // Check that the settings were saved to localStorage
    expect(mockStorage.setItem).toHaveBeenCalledWith('settings', expect.stringContaining('"themeMode":"dark"'));
    expect(mockStorage.setItem).toHaveBeenCalledWith('settings', expect.stringContaining('"language":"fr"'));
  });
});

...
export class IndexedDBService {
	...
	private notifyStorageUnavailable(
		reason: 'not_supported' | 'private_browsing' | 'quota_exceeded' | 'timeout' | 'corrupted' | 'error',
	): void {
	 ...
	 */
	private applyDatabaseMigrations(db: IDBDatabase, oldVersion: number, transaction: IDBTransaction): void {
	 ...
	 */
	public async getLogs(level?: 'debug' | 'info' | 'warn' | 'error' | 'critical', limit = 100): Promise<LogEntry[]> {
		await this.init()

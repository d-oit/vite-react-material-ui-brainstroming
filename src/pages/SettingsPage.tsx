...
const SettingsPage = ({ onThemeToggle, isDarkMode }: SettingsPageProps) => {
	...
	const [openRouterModel, setOpenRouterModel] = useState(settings.openRouterModel || 'anthropic/claude-3-opus')
	...
	const validateAws = (accessKeyId: string, secretAccessKey: string) => {
		...
		setAwsAccessKeyIdError(result.isValid ? '' : result.message.includes('access key ID') ? result.message : '')
				...
				<Box sx={{ width: '100%' }}>
					...
					<TabPanel value={tabValue} index={0}>
						<Box sx={{ mb: 4 }}>
							...
							<Accordion expanded={expanded.api} onChange={handleAccordionChange('api')} sx={{ mb: 2 }}>
								<AccordionSummary expandIcon={<ExpandMoreIcon />}>

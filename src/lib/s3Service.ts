 ...
 */
export const uploadProject = async (project: Project, onProgress?: ProgressCallback): Promise<string> => {
	// First check if S3 is enabled
	if (!isS3Enabled()) {
			...
			result.CommonPrefixes.map(async (prefix) => {
				...
				const versions =
					versionResult.Contents?.filter((obj) => {
						const key = obj.Key || ''
						return key.endsWith('.json') && !key.endsWith('latest.json') && !key.endsWith('metadata.json')
					})
						.map((obj) => {
							const key = obj.Key || ''

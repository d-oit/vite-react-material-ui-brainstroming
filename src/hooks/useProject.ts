...
export const useProject = ({ projectId, version, autoSave = true }: UseProjectProps = {}) => {
	...
	const createNewVersion = useCallback(async (): Promise<boolean> => {
		...
		try {
			...
			const versionStr = typeof project.version === 'string' ? project.version : String(project.version)
	...
	const updateNode = useCallback(
		(nodeId: string, updates: Partial<Node>): boolean => {
			...
			setProject((prev: Project | null) => {
				...
				return {
					...prev,
					nodes: prev.nodes.map((node: Node) => (node.id === nodeId ? { ...node, ...updates } : node)),
					updatedAt: new Date().toISOString(),
				}
			})
		...
		(nodeId: string): boolean => {
			...
			setProject((prev: Project | null) => {
				...
				return {
					...
					edges: prev.edges.filter((edge: Edge) => edge.source !== nodeId && edge.target !== nodeId),
					updatedAt: new Date().toISOString(),
				}
			})
	...
	const updateEdge = useCallback(
		(edgeId: string, updates: Partial<Edge>): boolean => {
			...
			setProject((prev: Project | null) => {
				...
				return {
					...prev,
					edges: prev.edges.map((edge: Edge) => (edge.id === edgeId ? { ...edge, ...updates } : edge)),
					updatedAt: new Date().toISOString(),
				}
			})
	...
	useEffect(() => {
		if (project === null || !('id' in project) || !('nodes' in project) || loading === true || isSaving === true) {
			return
		}

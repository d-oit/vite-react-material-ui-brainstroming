		...
		<Paper
					...
					{metrics.length === 0 ? (
						...
						<Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
							...
							<Chip
								...
								onClick={() =>
									setFilter(filter === MetricCategory.INTERACTION ? null : MetricCategory.INTERACTION)
								}
							/>
							<Chip
			...
			<TabPanel value={tabValue} index={2}>
				<Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
					<Typography variant="h6">{t('performance.dashboard.memoryUsage') || 'Memory Usage'}</Typography>

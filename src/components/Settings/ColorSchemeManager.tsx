...
const ColorPicker = ({ color, onChange }: { color: string; onChange: (color: string) => void }) => {
	return (
		<Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
			...
			/>
			<TextField size="small" value={color} onChange={(e) => onChange(e.target.value)} sx={{ width: 100 }} />
		</Box>
	)
}
...
export const ColorSchemeManager = () => {
	const { settings, updateSettings, colorSchemes, updateColorScheme, createColorScheme, deleteColorScheme } =
		useSettings()

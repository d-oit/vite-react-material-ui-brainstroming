...
export const NodePreferencesManager = () => {
	...
	const [touchOptimized, setTouchOptimized] = useState<boolean>(nodePreferences?.touchOptimized || false)

	if (!localPreferences) {
		return <Alert severity="warning">Node preferences could not be loaded. Please refresh the page.</Alert>
	}
	...
	return (
		<Box>
			<Box sx={{ p: { xs: 0, sm: 1 } }}>
				...
				<FormControlLabel
					control={<Switch checked={touchOptimized} onChange={(e) => setTouchOptimized(e.target.checked)} />}
				...
				<Box sx={{ mb: 3 }}>
					<FormControlLabel
						control={
							<Switch checked={touchOptimized} onChange={handleTouchOptimizationChange} color="primary" />
						}
						label={
				...
				<Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
					Customize the colors for each node type. These colors will be used as the default background color
					for nodes of each type.
				</Typography>

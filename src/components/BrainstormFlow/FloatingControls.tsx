...
export const FloatingControls: React.FC<FloatingControlsProps> = ({ position = { x: 100, y: 100 } }) => {
	...
	return (
		<SpeedDial
			...
			{nodeTypes.map(({ type, icon, label }) => (
				<SpeedDialAction key={type} icon={icon} tooltipTitle={label} onClick={() => handleAddNode(type)} />
			))}
		</SpeedDial>
	)
}

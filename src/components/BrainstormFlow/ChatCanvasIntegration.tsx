										...
										<ListItem
											...
											secondaryAction={
												<Button
													...
													color="error"
													onClick={() =>
														onUnlinkNodeFromMessage(selectedNode.id, message.id)
													}>
										...
										<ListItem
											...
											secondaryAction={
												<Button
													...
													color="error"
													onClick={() =>
														onUnlinkNodeFromMessage(node.id, selectedMessage.id)
													}>
													{t('flow.unlink') || 'Unlink'}
												</Button>
											}>
...
export const useChatCanvasIntegration = (props: ChatCanvasIntegrationProps) => {
	...
	return {
		...
		renderNodeLinkIndicator: (nodeId: string) => {
			const linkedMessages = props.links.filter((link) => link.nodeId === nodeId).map((link) => link.messageId)
		...
		renderMessageLinkIndicator: (messageId: string) => {
			const linkedNodes = props.links.filter((link) => link.messageId === messageId).map((link) => link.nodeId)

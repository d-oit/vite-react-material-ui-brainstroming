...
const mapStringToNodeSize = (sizeString?: 'small' | 'medium' | 'large'): NodeSize => {
  switch (sizeString) {
    case 'small': return NodeSize.SMALL;
    case 'large': return NodeSize.LARGE;
    case 'medium':
    default: return NodeSize.MEDIUM;
  }
};
    ...
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      ...
      <DialogContent>
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
          ...
          <TextField label="Label" value={label} onChange={e => setLabel(e.target.value)} fullWidth required />
          <TextField label="Content" value={content} onChange={e => setContent(e.target.value)} multiline rows={4} fullWidth />
          <Divider sx={{ my: 2 }} />
          <Typography variant="subtitle1" gutterBottom> Tags </Typography>
          <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 1 }}>
            <TextField label="Add Tag" value={newTag} onChange={e => setNewTag(e.target.value)} onKeyPress={e => { if (e.key === 'Enter') { e.preventDefault(); handleAddTag(); } }} fullWidth />
          ...
          <Paper sx={{ p: 2, mb: 2 }} elevation={0} variant="outlined">
            <Grid container spacing={2}> {/* Container Grid */}
              {/* Size selection */}
              <Grid xs={12} sm={6}> {/* Direct responsive props */}
                <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
                  ...
                  <Tooltip title="Toggle size options">
                    <IconButton size="small" onClick={() => setShowSizeSelector(!showSizeSelector)} aria-label="Toggle size options" color={showSizeSelector ? 'primary' : 'default'} sx={{ ml: 'auto' }} >
                      <FormatSizeIcon fontSize="small" />
                    </IconButton>
                  </Tooltip>
                </Box>
                {showSizeSelector ? (
                  <ToggleButtonGroup value={size} exclusive onChange={(e, newSize) => { if (newSize !== null) setSize(newSize); }} aria-label="Node size" fullWidth size={isMobile ? 'small' : 'medium'} >
                    <ToggleButton value={NodeSize.SMALL} aria-label="Small size"> <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}> <SmartphoneIcon fontSize="small" /> <Typography variant="caption">Small</Typography> </Box> </ToggleButton>
                    <ToggleButton value={NodeSize.MEDIUM} aria-label="Medium size"> <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}> <SmartButtonIcon /> <Typography variant="caption">Medium</Typography> </Box> </ToggleButton>
                    <ToggleButton value={NodeSize.LARGE} aria-label="Large size"> <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}> <LaptopIcon /> <Typography variant="caption">Large</Typography> </Box> </ToggleButton>
                  </ToggleButtonGroup>
                ) : (
                  <FormControl fullWidth>
                    <InputLabel id="node-size-label">Size</InputLabel>
                    <Select labelId="node-size-label" value={size} label="Size" onChange={e => setSize(e.target.value as NodeSize)} >
              ...
              <Grid xs={12} sm={6}> {/* Direct responsive props */}
                <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
                  ...
                  <Tooltip title="Reset to default color">
                    <span>
                      <IconButton size="small" onClick={() => setColor(undefined)} disabled={color === undefined} sx={{ ml: 'auto' }} aria-label="Reset to default color" >
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                  <Box sx={{ width: '100%', height: 36, borderRadius: 1, bgcolor: color ?? defaultNodeColor, border: '1px solid rgba(0, 0, 0, 0.2)', cursor: 'pointer', transition: 'all 0.2s ease', '&:hover': { boxShadow: 2, }, }}
                    onClick={() => {
                      if (isMobile) { setShowColorPicker(!showColorPicker); }
                      else {
                        const input = document.createElement('input');
                        input.type = 'color';
                        input.value = color ?? defaultNodeColor;
                        input.addEventListener('input', e => { setColor((e.target as HTMLInputElement).value); });
                        input.click();
                      }
                    }}
                    role="button" aria-label="Select color" tabIndex={0}
                  />
                      ...
                      {[ '#e3f2fd', '#e8f5e9', '#fff8e1', '#f3e5f5', '#0d47a1', '#1b5e20', '#f57f17', '#4a148c', ].map(presetColor => (
                        ...
                        <Tooltip key={presetColor} title={presetColor}>
                          <Box sx={{ width: 24, height: 24, borderRadius: '50%', bgcolor: presetColor, border: color === presetColor ? `2px solid ${theme.palette.primary.main}` : '1px solid rgba(0, 0, 0, 0.2)', cursor: 'pointer', transition: 'all 0.2s ease', '&:hover': { transform: 'scale(1.1)', }, }}
                            onClick={() => setColor(presetColor)} role="button" aria-label={`Use color ${presetColor}`} tabIndex={0}
                          />
                        </Tooltip>
                      ))}
                    </Box>
          ...
          {/* Node preview */}
          <Box sx={{ mb: 2, p: 2, border: '1px dashed rgba(0, 0, 0, 0.2)', borderRadius: 1, bgcolor: 'background.paper', display: isMobile ? 'none' : 'block', }} >
            <Typography variant="caption" color="text.secondary" gutterBottom> Preview </Typography>
            <Box sx={{ width: nodePreferences ? nodePreferences.nodeSizes[size].width : 200, backgroundColor: color ?? defaultNodeColor, borderLeft: `4px solid ${theme.palette.primary.main}`, borderRadius: 1, p: 1, boxShadow: 1, fontSize: nodePreferences ? `${nodePreferences.nodeSizes[size].fontSize}rem` : '1rem', }} >
              <Typography variant="subtitle2" sx={{ fontWeight: 'bold', fontSize: nodePreferences ? `calc(${nodePreferences.nodeSizes[size].fontSize}rem * 1.1)` : '1.1rem', }} >
                {label || 'Node Title'}
              </Typography>
              {tags.length > 0 && (

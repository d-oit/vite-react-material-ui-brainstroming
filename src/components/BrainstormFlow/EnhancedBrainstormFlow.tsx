  ...
  ({
    ...
    const handleNodeDeleteRequest = useCallback(
      (nodeId: string, event?: React.MouseEvent) => {
        ...
        void loggerService.info(`Node deletion requested for node ${nodeId}`);
    ...
    const processNodes = useCallback(
      (nodesToProcess: Node[]): Node[] => {
        const metricId = performanceTracker.startMeasure('ProcessNodes', MetricCategory.RENDER, {
        ...
        const result = nodesToProcess.map(node => ({
          ...
          data: {
            ...
            onSaveDirectEdit: (id: string, newData: { label: string; content: string }) => {
              ...
              // Log the update
              void loggerService.info('Node directly edited', {
                nodeId: id,
    ...
    const saveFlow = useCallback(() => {
      if (onSave) {
        const metricId = performanceTracker.startMeasure('SaveFlow', MetricCategory.INTERACTION, {
    ...
    const handleAddNodesFromChat = useCallback(
      (nodeDatas: NodeData[]) => {
        if (reactFlowInstance === null || !Array.isArray(nodeDatas) || nodeDatas.length === 0)
          return;
    ...
    return (
      ...
      >
                  ...
                  <Checkbox
                    onChange={e => {
                      ...
                      if (e.target.checked) {
                        ...
                        void loggerService.info('User enabled skipDeleteConfirmation setting');
                      }
                    }}
                    color="primary"
                  />
        ...
        <EnhancedControlsPanel
          ...
          onToggleGrid={() => setShowGrid(!showGrid)}
          onZoomChange={(zoom) => reactFlowInstance?.setZoom(zoom)}
          hasSelection={!!selectedNode}
          canUndo={false}
          canRedo={false}
          hasCopiedItems={false}
          showGrid={showGrid}
          currentZoom={reactFlowInstance?.getZoom() || 1}
        />
      </Box>
    );
  }
